...
void main()
{
	...
	if (lod - int(lod) == 0)
	{
		lookup = calculate_lookup_pixel(floor_w, tile_sample_count[0], true, vec2(spatialCoordinate), ceil(lod), int(lod), vec2(0, 0), vec2(0, 0));
		if (lookup >= 0)
		{
			if (histogramIndexCentral == 0)  //the ray didn't hit anything, we count the ray misses in bin 0,0 of each pixel
			{
			...
			if (histogramIndexCentral + 1 < HistogramHeight * HistogramWidth * 2)
			{
				add_sample(histogramIndexCentral + 1, float((bilinearWeights.x) * (1.0 - bilinearWeights.y) * unit), lookup);
				//ndfVoxelData.histograms[lookup + histogramIndexCentral + 1] += float((bilinearWeights.x) * (1.0 - bilinearWeights.y) * unit);
			}
			if (histogramIndexCentral + HistogramWidth * 2 < HistogramHeight * HistogramWidth * 2)
			{
				add_sample(histogramIndexCentral + HistogramWidth * 2, float((1.0 - bilinearWeights.x) * (bilinearWeights.y) * unit), lookup);
				//ndfVoxelData.histograms[lookup + histogramIndexCentral + HistogramWidth*2] += float((1.0 - bilinearWeights.x) * (bilinearWeights.y) * unit);
			}
			if (histogramIndexCentral + HistogramWidth * 2 + 1 < HistogramHeight * HistogramWidth * 2)
			{
				add_sample(histogramIndexCentral + HistogramWidth * 2 + 1, float((bilinearWeights.x) * (bilinearWeights.y) * unit), lookup);
	...
	else
	{
		...
		//sample floor and ceil
		{
			...
			//lookup = calculate_lookup_pixel(ceil_w, tile_sample_count[4], false, vec2(spatialCoordinate), ceil(lod), ceil(lod), vec2(0, 0), vec2(0, 0));
