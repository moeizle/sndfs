...
{
	int samples[]; // This is the important name (in the shader).
};
...
void main()
{
	...
	rayOffset.x *= 1.0f/viewportWidth;
	rayOffset.y *= 1.0f/viewportWidth;
	//calculate rayoffset
	...
	for (int y = 0; y < multiSampling; ++y)
	{
		for (int x = 0; x < multiSampling; ++x)
		{
			...
			//old
			//lookup = (spatialCoordinate.y * VolumeResolutionX + spatialCoordinate.x) * HistogramHeight * HistogramWidth;
			...
			if (lod - int(lod) == 0)
			{
				...
				//else if (histogramIndexCentral == 0)  //the ray didn't hit anything, we count the ray misses in bin 0,0 of each pixel
				//{
				//	ndfVoxelData.histograms[lookup + histogramIndexCentral] += float(unit);
				//}
			...
			else
			{
				...
				{
					lookup = calculate_lookup_pixel(floor_w, tile_sample_count[0], true, vec2(spatialCoordinate), ceil(lod), int(lod), vec2(0, 0), -viewportWidth*rayOffset);
