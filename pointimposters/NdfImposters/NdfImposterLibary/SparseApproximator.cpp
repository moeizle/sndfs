...
ApproximationResult SparseApproximator<DataType>::Approximate(glm::ivec2 histogramResolution, glm::ivec2 spatialResolution, int maxClusters, std::vector<float> data, int emIterations) {
	...
	for(int histogramIndex = 0; histogramIndex < spatialResolution.x * spatialResolution.y; ++histogramIndex) {
		...
		Helpers::Approximation::ExpectationMaximization<3> approximation(ssboBegin, ssboEnd, { histogramResolution.x, histogramResolution.y, 1 }, maxClusters, emIterations);
	...
	std::cout << "Expectation maximization finished" << std::endl;
